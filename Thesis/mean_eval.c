        ...
        )
{
    ...
    int **list_nos, *mask;
    int cont, no_row, no_col, nro_limiar, raio, N_vicinities, N_nos, set_size;
    int lin, col, li, grau, tam_mask, i , j;
    double edge_sum, mean;
    ...
    no_row = mxGetM(prhs[0]);    /* number of rows of the image */
    no_col = mxGetN(prhs[0]);    /* number of columns of the image */
    pr = mxGetPr(prhs[0]);
    

    raio = (int) mxGetScalar(prhs[1]);
    set_size = (int) mxGetScalar(prhs[2]);
    
    
    N_nos = no_row * no_col;
    N_vicinities = No_Vicinities(raio);  // number of neighbours in r radius
    tam_mask = (int) N_vicinities / 2;
    mask = MaskCircumference(raio, tam_mask, no_row);
